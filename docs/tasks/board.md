...
| TK-S2 | Dựng + nghiệm thu 2 VPS cùng provider/gói/region | A | 12/08 | ĐANG LÀM | chore/vps-nghiem-thu-19-08 | #12 | Đã dựng + 6/6 nghiệm thu + merge 19/08; còn 3 việc tay người dùng: snapshot `clean-docker-19-08`, nạp pubkey B, chép DC/hạn thanh toán vào `docs/08` mục 0 |
...
| TK-A10 | M1 chẩn đoán lỗi kết nối VPS: probe TCP + 5 lớp lỗi + gợi ý sửa tiếng Việt | A | 21/08 | ĐANG LÀM | feat/m01-connect-diagnostics | — | Bài toán mở đầu demo 24/08 (case mẫu: firewall WiService chặn SSH). Mở rộng `VpsConnectionCheck.diagnosis` (đổi contract → DECISIONS). Hồ sơ: `tk-a10-m1-diagnostics.md` |
