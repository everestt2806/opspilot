...
| TK-A14 | Dashboard v1: tổng quan VPS + lịch sử + log deploy live | A | 23/08 | CHỜ REVIEW | feat/ui-dashboard-v1 | — | Ops dashboard cho demo (không chart metric — collector lùi W2). Xong cả 3 chunk: `history:list` IPC + DashboardPage + Deploy Log xterm + HistoryPage (filter + drawer key–value), 124 test. |
