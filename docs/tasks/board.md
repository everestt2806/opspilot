...
| ------ | -------------------------------------------------------------------- | --------------------- | --------- | ---------- | --------------------------- | --------------------------- | ------------------------------------------------------------------------------------- |
| TK-B12 | Fix migrate Express fail ở VERIFY: health race sau pg_restore       | B (A review)          | 17/09     | ĐANG LÀM  | `fix/migrate-verify-health-race` | Stacked trên TK-B10, gộp PR | Chỉ health fail, marker PASS; [hồ sơ](tk-b12-migrate-verify-health.md) |
| TK-B11 | Tính năng xóa app (xóa mềm + dọn VPS + trả port, giữ data ML)       | B (A review)          | 24/09     | BACKLOG    | `feat/app-delete`           | Sau TK-B12                  | Kế hoạch đã chốt trong tk-file; [hồ sơ](tk-b11-app-delete.md) |
| TK-B10 | Fix migrate mất build-time env khi rebuild trên VPS đích            | B (A review)          | 17/09     | ĐANG LÀM  | `fix/migrate-build-time-env`| Chờ A duyệt push            | Bug rehearsal demo 15/09: VITE_API_URL rơi về localhost; [hồ sơ](tk-b10-migrate-build-env.md) |
