...
async function waitFinished(win, deploymentId, timeoutMs = 30 * 60_000) {
  ...
  while (Date.now() < deadline) {
    ...
    for (const event of events) {
      if (
        !evidence.events.some(
          (item) =>
            item.deployment_id === event.deployment_id &&
    ...
    const finished = events.find(
      (event) =>
        event.type === "finished" && event.deployment_id === deploymentId,
    );
      ...
      win.webContents.once("did-finish-load", async () => {
        try {
          await win.webContents.executeJavaScript(
            `window.__c01Events = []; void window.api.on('deploy:event', (event) => window.__c01Events.push(event)); true`,
          );
          ...
          if (firstFinished.status !== "running")
            throw new Error(`v1 finished ${firstFinished.status}`);
          ...
          if (secondFinished.status !== "running")
            throw new Error(`v2 finished ${secondFinished.status}`);
          ...
          const items = JSON.parse(
            remoteCurl(current.host_port, "/items?limit=1&offset=1000"),
          );
          evidence.deployments = {
            v1: first,
            v1Finished: firstFinished,
            marker,
            v2: second,
            v2Finished: secondFinished,
            current,
            versions,
            meta,
          ...
          if (
            ...
            !items.some((item) => item.name === "C01 marker 2026-09-11")
          ) {
            throw new Error(
              "PostgreSQL marker was not preserved after redeploy",
            );
          }
          console.log(
            JSON.stringify({
              type: "C01_REDEPLOY_VERIFIED",
              current,
              versions,
              meta,
              itemCount: items.length,
