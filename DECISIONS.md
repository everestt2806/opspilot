...
| 2026-08-19 | [ui] | Thêm devDependencies `@testing-library/react` + `@testing-library/dom` + `jsdom` cho component test renderer (chỉ dùng trong test, không vào bundle) | DoD TK-B7 yêu cầu "component test làm bằng chứng" cho 4 state; danh sách dep đã duyệt chưa có lib test DOM nào | `app/package.json` (devDeps), `vitest.config.ts`, test đặt trong `app/src/renderer/src/**` |
| 2026-08-20 | [ml] | OCSVM `gamma` từ `'scale'` (1/20) xuống **`0.001`** | Đo 80 seeds trên dữ liệu giả: 'scale' khiến kernel quá khít với 201 vector/20 chiều — hầu hết mẫu unseen bình thường chạm trần score 1.0 (FP ~71%); 0.001 hạ FP về ~40% mà anomaly rõ vẫn 1.0 ở mọi run | `ml-service/models/ocsvm.py`, `docs/prompts/m07-ml-service.md` |
| 2026-08-20 | [ml] | **Giữ nguyên** chuẩn hoá min-max p1/p99 tập train cho iforest/ocsvm dù FP trên unseen còn cao (đo giả lập: iforest ~41%, ocsvm ~40%); chỉ hạ `gamma` | Đã thử n_estimators/max_samples/contamination/OOF-calibration/rolling-percentile — cải thiện nhiều nhất cũng chỉ về ~25-35% FP nhưng phải thêm trạng thái mới (rolling pool) hoặc lệch xa chữ M7; quyết định đánh giá FP thật để ở W9 khi có dữ liệu collector thật, B chủ trì | `ml-service/models/`, `tests/test_service_endpoints.py`, nhật ký `docs/tasks/tk-a6-m7.md` |
| 2026-08-24 | [ssh] | Thêm channel `vps:scan` (quét môi trường VPS: ssh/docker/compose/node/git/workdir, kèm `VpsScanItem`/`VpsScanResult`) và 9 channel `db:*` (quản trị Postgres trên VPS qua psql-over-ssh, kèm types `Db*`) vào `IpcInvokeMap` | Panel TK-B9 cần nút quét môi trường và tab Database; B dựng UI trước theo thoả thuận "UI trước, backend sau" nên handler `db:*` nối sau — UI hiện lỗi trung thực "channel chưa nối" thay vì dựng nút giả. Không đổi kênh cũ nên payload hiện có không vỡ | `docs/contracts/ipc-contract.ts`, `app/src/shared/ipc.ts`, `app/src/main/vps/scanService.ts`, tab Database + card scan UI |
