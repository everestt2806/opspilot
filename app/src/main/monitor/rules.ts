...
export function evaluateRule(sample: MetricSample, setting: MonitorSetting): { violated: boolean; reasons: string[] } {
  ...
  if (sample.latency_ms !== null && sample.latency_ms > setting.rule_latency_ms) reasons.push('latency_ms')
  if (sample.http_error_rate !== null && sample.http_error_rate > setting.rule_error_rate) reasons.push('http_error_rate')
